...
def warpImg(image,affineMatrix,dimension):
    ...
    # print(affineMatrix.shape)
    # print(affineMatrix[-dimension:])
    # print(affineMatrix[0:dimension*dimension])
    # print(affineMatrix)
...
def warpTensors(movingImgs,affineMatrices,dimension):
    ...
    for i in range(movingImgs.size(0)):
        ...
        # print(movImg.GetSize())

        warpResult=warpImg(movImg,am_n,dimension)
        # sitk.WriteImage(warpResult,'%04d'%i+'.nii')
...
if __name__ == "__main__":
    movingImgPath='/home/customer/kexin/pytorchTest/allDataset/0824CMTK_AM_TM3_checkAffineResult_noGoodAffine/test/movingImg/r20210824_X1_0037.nii'
    dimension=3
    movingImg=sitk.ReadImage(movingImgPath)
    # affineMatrix=np.array([ 1, 0, 0,  0, 1, 0, 0, 0, 1, 10, 0, 0])
