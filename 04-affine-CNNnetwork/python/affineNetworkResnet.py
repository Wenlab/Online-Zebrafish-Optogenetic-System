...
class ProsRegNet(nn.Module):
    ...
    def forward(self, moving,fix):
        # do feature extraction
        feature_A = self.FeatureExtraction(moving)
        feature_B = self.FeatureExtraction(fix)
        # normalize
        ...
        if theta.shape[1] == 6:
            temp = torch.tensor([1.0,0,0,1.0,0,0])
            adjust = temp.repeat(theta.shape[0],1)
            adjust = adjust.to("cuda:2")
            theta = 0.1*theta + adjust
            theta = theta.reshape(theta.size()[0],6)
            theta = theta.to("cuda:2")
