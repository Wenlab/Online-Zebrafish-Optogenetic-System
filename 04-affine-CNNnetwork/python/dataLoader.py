...
class AffineRegistrationDataset(torch.utils.data.Dataset):
    def __init__(self,data_path,transforms=None):
        ...
        # self.all_movingImgName.sort(key=lambda x:int(x[:-4]))
        self.all_movingImgName.sort()

        self.all_fixImgName=os.listdir(self.fixImg_path)
        # self.all_fixImgName.sort(key=lambda x:int(x[:-4]))
...
def show_batch_images(sample_batch,batchSize,filename):
    ...
    for i in range(batchSize):
        label_ = label[i].item()
        # image_ = np.transpose(movingImg[i], (1,0))
...
if __name__ == "__main__":
    ...
    #     ncc1=ncc_losses_global(fixes,movings)
    #     ncc2=ncc_losses_global(fixes,result)
    #     n1=ncc1+n1
    #     n2=ncc2+n2
    #     print(ncc1)
    #     print(ncc2)
