...
void T2Cam_InitializeCamData(CamData* MyCamera,int _handle) {
    ...
    //Mono16格式一个像素占2Byte, short也占2Byte, static_cast<size_t>(MyCamera->ImageHeight*MyCamera->ImageWidth)是照片像素的数量, 所以申请这样一个short数组
...
void deleteBuffers(CamData* MyCamera){
  ...
  //for (int i=0; i < NumberOfBuffers; i++) { delete[] MyCamera->AcqBuffers[i]; } 
  //delete[] MyCamera->AlignedBuffers; 
  //delete[] MyCamera->AcqBuffers;
