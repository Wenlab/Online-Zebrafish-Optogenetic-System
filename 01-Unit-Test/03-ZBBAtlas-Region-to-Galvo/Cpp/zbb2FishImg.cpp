...
void zbb2FishImg::getRegionFromUser(cv::Rect Reg)
{
	...
	//Converting areas to coordinates
...
//Get the rotation angle information from the original image to the rotated image
...
//The information obtained using the above functions
...
vector<vector<vector<string>>> zbb2FishImg::makeZbbMapVec(vector<pair<string, Point>> zbbMapPair)
{
	...
	for (int i = 0; i < zbbMapPair.size(); i++)
	{
		...
		zbbMapVec[temp.second.x - 1][temp.second.y - 1].push_back(temp.first);   //The coordinates saved by matlab start from 1
	}
...
//Search for the name of the contained brain region based on the region coordinates
...
//Find the inverse matrix of the affine matrix
...
//Applying affine matrices to coordinates
...
//Applying the rotation matrix to coordinates
