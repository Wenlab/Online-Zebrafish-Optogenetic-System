function afterRotation=rotation3DFishImg(ObjRecon)
    ...
    % 1st Rotation 
        ...
        afterRotation=imrotate(ObjRecon,-RotateAngle(1),'bicubic', 'crop');  %rotation一次
        ...
        if CentroID(2) < (Y/2)
            afterRotation = imrotate(afterRotation, 180,'bicubic', 'crop');  %rotation两次
        end
        ...
        if(height(statsX)>0)
            ...
            afterRotation = imrotate(afterRotation,-RotationAngle(2),'bicubic', 'crop');  %rotation三次
            afterRotation=permute(afterRotation,[2 3 1]);
%             afterRotation = imrotate(afterRotation,180,'bicubic', 'crop');  %rotation四次
